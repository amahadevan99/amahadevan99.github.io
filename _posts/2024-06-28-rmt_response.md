...
Here we will exploit the correspondence between the response function of a dynamical system and the resolvent of a matrix in order to calculate the spectral densities of two famouse random matrix ensembles. The results of these calculations are the same as in [these notes](https://amahadevan99.github.io/files/marchenko_pastur.pdf) (which also contain a summary of why we care about the resolvent at all) but the means are different (and perhaps more intuitive, at least from a physics perspective). The central correspondence is that _the resolvent of a matrix_ $$M$$ _is the same as a Laplace transform of the response function of a dynamical system_ $$x_i = \sum_j M_{ij} x_j$$. This explains why resolvents are referred to as Greens functions sometimes: the Greens function of an equation or set of equations is nothing but its response function. We will use the dynamic cavity method to calculate the response function of a particular dynamical matrix, and therefore find its spectrum.

Referring [here](https://amahadevan99.github.io/files/marchenko_pastur.pdf) for more details, we recall that if we know resolvent, defined as

$$ g(z) = \frac{1}{N}{\rm E}\left[\Tr \frac{1}{z-A}\right],$$

then (if $$A$$ is from an ensemble with real eigenvalues) the spectral density of the ensemble from which $$A$$ is drawn is given by

$$ \rho(x) = \lim_{\epsilon\to 0^+} \frac{1}{\pi}\operatorname{Im} g(x-i\epsilon).$$

We also recall that response functions capture how a function of interest changes when an external field is applied at a prior time. For stationary processes the response function depends only on a _time difference_ and if we have a process $$x(t) = f(x,t) + h(t)$$ for external field $$h(t)$$, then the response function $$\chi(\tau)$$ for time difference $$\tau$$ comes in as
...
where $$A$$ is drawn from the ensemble whose spectral density we wish to compute, and we use Einstein summation convention. Let's take it to be the Gaussian orthogonal ensemble (GOE) which is comprised of symmetric matrix with gaussian entries of variance $$1/N$$ for off-diagonal and $$2/N$$ for diagonal entries. What happens if we add a new dimension into our vector $$x$$, and concomitantly into our matrix $$A$? The dynamics of $$x_0$$ will be given by

$$ \dot x_0 = A_{0j}(x_j+\delta x_j) + A_{00}x_0,$$

where $$\delta x_j$$ measures the change in the the other components of the $$x$$, due to the addition of $$x_0$. These in turn are dynamical variables given (according to the definition of the response function) by
...
In other words, as we stated, {\it the Laplace transform of the response function of the dynamics generated by the matrix is the same as the resolvent of the matrix.}
...
This formula is valid for $$0\leq q\leq 1$; for $$q>1$$ we additionally get a $$(1-1/q)\delta(x)$$ contribution from the zero-eigenvalues.
